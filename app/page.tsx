...
function AdminPanelEnhanced({ cohortId, onClose }: { cohortId: string; onClose: () => void }) {
  ...
  const load = useCallback(async () => {
    if (!supabase) return;
    const [memberResult, reportResult, deletionResult] = await Promise.all([
      supabase.from("memberships").select("user_id,role,active,profiles(id,full_name,initials,avatar_url)").eq("cohort_id", cohortId).order("created_at"),
      supabase.from("content_reports").select("id,case_id,comment_id,reason,created_at,resolved_at").is("resolved_at", null).order("created_at"),
      supabase.from("account_deletion_requests").select("user_id,requested_at,status").neq("status", "completed").order("requested_at"),
    ]);
    const reportRows = (reportResult.data ?? []) as AdminReport[];
    ...
    setMembers((memberResult.data ?? []) as unknown as AdminMember[]);
    setReports(reportRows.map((report) => ({ ...report, case: report.case_id ? caseMap.get(report.case_id) : undefined })));
    setDeletions((deletionResult.data ?? []) as DeletionRequest[]);
    if (reportResult.error || deletionResult.error) setMessage("Algumas filas administrativas exigem a migration remota mais recente.");
  }, [cohortId]);
